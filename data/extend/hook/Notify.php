...
use data\service\SendMessageSy;
...
class Notify {
    ...
    private function getShopNotifyInfo($shop_id, $website_id) {
        ...
        #查看邮箱是否开启
        $email_info = $configSer->getConfig($shop_id, "EMAILMESSAGE", $website_id);
        if (!empty($email_info)) {
        ...
        $mobile_info = $configSer->getConfig($shop_id, "MOBILEMESSAGE", $website_id);
        if (!empty($mobile_info)) {
            ...
            if (!empty($value)) {
                ...
                $this->ali_use_type = $mobile_array["user_type"];//1阿里云 2微商来短信 3阿里云大于
                $this->int_sign_name = $mobile_array["int_sign_name"];
                $this->international = $mobile_array["international"];
                $this->ali_use_type_start = $mobile_array["user_type"];
                $this->order_mobile = $mobile_array["order_mobile"];//订单通知手机
                if (empty($this->ali_use_type)) {
                    $this->ali_use_type = 2;
                }
            }
        }
        $master_info = $configSer->getConfigMaster(0, "MOBILEMESSAGE", 0);//a端短信配置
        if (!empty($master_info)) {
            $value_master = $master_info["value"];
            if (!empty($value_master)) {
                $master_array = $value_master;
                $this->userid = $master_array["userid"];
                $this->username = $master_array["username"];
                $this->password = $master_array["password"];
                $this->sy_username = $master_array["sy_username"];
                $this->sy_password = $master_array["sy_password"];
                if($master_array["user_type"] == '3' && $this->ali_use_type == 2){
                    $this->ali_use_type = 3;
                }
            }
        }
    ...
    public function sendMessage($messageType = '', $mobile = '', $params = array(), $shop_id = 0, $website_id = 0) {
        ...
        }elseif($this->ali_use_type == 3 && ($countryCode == '86' || $countryCode == '')){
            return $this->sendMessageBySy($messageType, $mobile, $params, $shop_id, $website_id);
        }else{
            $this->ali_use_type = $this->ali_use_type_start;
        }
        ...
        switch ($messageType) {
            ...
            case 'send_password':
                    ...
                    break;
            case 'send_electroncard_msg':
                $sms_params = array(
                    "goodsname" => $params['goodsname'],
                    "electroncardmsg" => $params['electroncardmsg'],
                );
                break;
            case 'send_give_coupon_msg':
                $sms_params = array(
    ...
    public function sendMessageByJd($messageType = '', $mobile = '', $params = array(), $shop_id = 0, $website_id = 0) {
        ...
        switch ($messageType) {
	        ...
	        case 'send_password':
                ...
                break;
            case 'send_electroncard_msg':
                $sms_params = str_replace('${goodsname}', $params['goodsname'], $template_obj['template_content']);
                $sms_params = str_replace('${electroncardmsg}', $params['electroncardmsg'], $sms_params);
                break;
            case 'send_give_coupon_msg':
                $sms_params = str_replace('${couponnum}', $params['couponnum'], $template_obj['template_content']);
                $sms_params = str_replace('${giftnum}', $params['giftnum'], $sms_params);
                break;
        }
        if (!$mobile) {
            $this->result["code"] = -1;
    ...
    public function sendMessageBySy($messageType = '', $mobile = '', $params = array(), $shop_id = 0, $website_id = 0) {
        ...
        if (!$this->sy_username || !$this->sy_password) {
            $this->result["code"] = -1;
            $this->result["message"] = "短信配置信息有误,请联系平台!";
            return $this->result;
        }
        ...
        if($template_obj){
            switch ($params['messageType']) {
                ...
                case 'area_adjust_notice':
                    ...
                    break;
                case 'electroncard_wx_notice':
                    $sms_params = $params['message_str'];
                    break;
                case 'appointgivecoupon_wx_notice':
                    $sms_params = $params['message_str'];
                    break;
            }
...
//        Db::query($sql);
    ...
    public function orderRemindBusinessBySms($params = null) {
        ...
        if (!empty($order_id)) {
            ...
            $order_detial = $vsl_order->getInfo(["order_id" => $order_id],'shop_id,order_money');
            $userSer = new UserSer();
            if(getAddons('shop', $website_id) && $order_detial['shop_id'] > 0){
                $mobile = $userSer->getWebsiteOrShopOrderMobile($website_id, $order_detial['shop_id']);//优先订单通知手机
            }else{
                $mobile = $userSer->getWebsiteOrShopOrderMobile($website_id);//优先订单通知手机
            }
            $sms_params = array(
    ...
    public function orderRefoundBusinessBySms($params = null) {
        ...
        if (!empty($order_id)) {
            $vsl_order = new VslOrderModel();
            $order_detial = $vsl_order->getInfo(["order_id" => $order_id],'shop_id');
            $userSer = new UserSer();
            if(getAddons('shop', $website_id)){
                $mobile = $userSer->getWebsiteOrShopOrderMobile($website_id, $order_detial['shop_id']);//优先订单通知手机
            }else{
...
//        $refund_type = reset($order_goods_obj)['refund_type'];
    ...
    public function emailSend(array $params) {
        ...
        switch ($params['template_code']) {
            ...
            case 'create_order':
                // 创建订单
                $order_info = $order_model::get(['order_id' => $params['order_id']]);
                $content = htmlspecialchars($template_email_obj['template_content']);
                $user_name = $params['user_name'] ?: ($order_info->buyer->nick_name ?: $order_info->buyer->user_name);
				...
				$title_tem = $template_email_obj['template_title'];
				$title_tem = str_replace('${shopname}', $website_info['mall_name'], $title_tem);
                $title_tem = str_replace('${username}', $user_name, $title_tem);
                $title_tem = str_replace('${ordermoney}', $order_money, $title_tem);
                $title_tem = str_replace('${timeout}', $params['time_out'], $title_tem);
                // {} in chinese
                $title_tem = str_replace('$｛shopname｝', $website_info['mall_name'], $title_tem);
                $title_tem = str_replace('$｛username｝', $user_name, $title_tem);
                $title_tem = str_replace('$｛ordermoney｝', $order_money, $title_tem);
                ...
                $result = $send->email($to_email, $title_tem, $content);
                if (!$result) {
                    $this->result['code'] = -1;
                    $this->result['message'] = '发送失败';
                }
     ...
     * @param string $params
     */
    public function sendElectroncardMsgBySms($params = null) {
        $user = new UserModel();
        $user_info = $user->getInfo(["uid" => $params['uid']], "user_tel,website_id");
        $user_tel = $user_info["user_tel"];
        $sms_params = array(
            "goodsname" => $params['goods_name'],
            "electroncardmsg" => $params['content'],
        );
        return $this->sendMessage('send_electroncard_msg', $user_tel, $sms_params, $params["shop_id"], $user_info['website_id']);
    }
     ...
     * @param string $params
     */
    public function sendGiveCouponMsgBySms($params = null) {
        $user = new UserModel();
        $user_info = $user->getInfo(["uid" => $params['uid']], "user_tel,website_id");
        $user_tel = $user_info["user_tel"];
        $sms_params = array(
        ...
        return $this->sendMessage('send_give_coupon_msg', $user_tel, $sms_params, $params["shop_id"], $user_info['website_id']);
    }
}
