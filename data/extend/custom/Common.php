...
class Common
{
    ...
    /**
     * @pc端装修模板列表 - 组装pc模板数据（根据模板的数据进行查询处理）
     */
    function get_seller_template_info($template_name = '', $theme = '', $status = 1)
    {
        ...
        foreach ($ext as $val) {
            ...
            if (file_exists(ROOT_PATH .  'public/static/custompc/data/web_'.$this->website_id.'/shop_'.$this->instance_id.'/'.$theme.'/' . $template_name . '/screenshot.' . $val)) {
                if ($theme == 'goods_templates') {
                    $info['screenshot'] = 'public/static/custompc/data/default/templates/goods_templates/goods_templates_tpl_1/screenshot.' . $val;
                } else {
                    $info['screenshot'] = 'public/static/custompc/data/web_'.$this->website_id.'/shop_'.$this->instance_id.'/'.$theme.'/' . $template_name . '/screenshot.' . $val;
                }
                break;
            }else{
                $sys_template_tpl = $theme .'/'.$theme.'_tpl_1';
                if (file_exists(ROOT_PATH.'public/static/custompc/data/default/templates/'.$sys_template_tpl.'/screenshot.' . $val)) {
                    $info['screenshot'] = 'public/static/custompc/data/default/templates/'.$sys_template_tpl.'/screenshot.' . $val;
					break;
                }
            }
        }
        ...
        if ($info['screenshot']) {
            $info['screenshot'] = __IMG($info['screenshot']);
        }
        return $info;
    }
    /**
...
//        $filter['page_arr'] = $this->seller_page($filter, $filter['page'], 14);
    ...
    function getGoodsList($where = '', $sort = '')
    {
        $goods_list = \think\Db::table("vsl_goods ng")->join('vsl_shop ns','ng.shop_id = ns.shop_id and ns.website_id = ng.website_id', 'left')->join('sys_album_picture sap','ng.picture = sap.pic_id', 'left')->where($where)->field('ng.market_price, ng.goods_name, ng.goods_id, ng.price,sap.pic_cover_micro,sap.pic_cover_mid,sap.pic_cover_small,ng.promotion_price,ns.shop_name')->order($sort)->select();
...
//            $image = 'http://pic.vslai1.com/' . $image;
     ...
     * @param $dest
     */
    function copydir($source, $dest, $name ='')
    {
        ...
        while (($item = @readdir($handle)) !== false) {
            ...
            //处理名字
            if ($name && $item =='tpl_info.txt'){
                $this->setUpTplInfoOfName($name,$_dest);
            }
        }
        @closedir($handle);
        return true;
     ...
     * @param $dest
     */
    public function setUpTplInfoOfName ($name,$dest)
    {
        $end = '------tpl_info------------';
        $tab = "\n";
        $html = $end . $tab . $name . $tab . 'tpl url：'. $tab . 'description：' . $tab . $end;
//        $this->com->write_static_file_cache('tpl_info', iconv('UTF-8', 'GB2312', $html), 'txt', $file_dir . '/');
         @file_put_contents($dest, iconv('UTF-8', 'GB2312', $html), LOCK_EX);
    }
