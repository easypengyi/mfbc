...
class WxPayConfig
{
    ...
    function __construct($website_id = 0, $is_mp = 0)
    {
        
        $this->weixin = new WeiXinPay($website_id);
        // 微信 - 小程序基础配置
        if ($is_mp == 1) {
            $this->appid = $this->weixin->getMpPayAppid();
            $this->MCHID = $this->weixin->getMpPayMchid();
            $this->key = $this->weixin->getMpPayMchkey();
        } else {
            // 微信 - 基础配置
