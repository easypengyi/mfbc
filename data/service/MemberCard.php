...
class MemberCard extends BaseService
{
     ...
     * @return int
     */
    public function saveData($order_id,$instance_id=0)
    {
        ...
        try {
            ...
            if ($order_goods_info) {
                ...
                if($order_goods_info['website_id']){
                    $is_ssl = \think\Request::instance()->isSsl();
                    $protocol = $is_ssl ? "http://" : "https://";
                    $website = new WebSiteModel();
                    $realm_info = $website->getInfo(['website_id'=>$order_goods_info['website_id']],'realm_ip');
                    $realm_ip = $realm_info['realm_ip'];
                    if($realm_ip){
                        $url = $protocol.$realm_ip;
                    }else{
...
//                    $url = __URL('clerk/pages/verify/cardvoucher?code=' . $code);
                    $url = $url.'/clerk/pages/verify/cardvoucher?code=' . $code;
...
//                        $url = __URL('clerk/pages/verify/cardvoucher?code=' . $code);
    ...
    public function getCardDetail($card_id, $card_code = '', $wx_card_id = '')
    {
        ...
        if (!empty($card_info)) {
            ...
            if(!$card_info['card_code']){
                $code = 'B' . rand(100000, 999999) . rand(10000, 99999);
                $input['card_code'] = $code;
                $url = __URLS('clerk/pages/verify/cardvoucher?code=' . $code);
                $result = $member_card->save($input,['card_id' => $card_info['card_id']]);
                if ($result > 0) {
                    $qrcode = getQRcode($url, 'upload/' . $this->website_id . '/' . $this->instance_id . '/goods_card_qrcode', 'goods_card_qrcode_' . $result);
                    $member_card->save(['card_codeImg' => $qrcode], ['card_id' => $card_info['card_id']]);
                }
                $card_info['card_code'] = $code;
                $card_info['card_codeImg'] = $qrcode;
            }
            ...
            if (getAddons('shop', $this->website_id)) {
                $shop = new Shop();
                $shop_info = $shop->getShopInfo($card_info['shop_id'], 'shop_name');
