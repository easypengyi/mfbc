...
class BaseService
{
    ...
    private function init()
    {
        ...
        $this->mall_name = Session::get($model . 'mall_name');
        $this->supplier_id = Session::get($model.'supplier_id')?:0;
        $this->port = Session::get($model . 'port');
    }
    /**
     * 把返回的数据集转换成Tree
     ...
     * @return array
     */
    public function posterImage($poster, $poster_id, $type = 'task_poster', $open_id, $goods_id = 0, $uid = 0, $mpController = '')
    {
        try {
            // 区分小程序
            $is_mp = false;
            if ((isset($poster['is_mp']) && $poster['is_mp'] == 1) || config('project') == 'shopdds') {
                $is_mp = true;
            }
...
//                return ['code' => -1, 'message' => '空的用户信息'];
            ...
            if($type == 'poster' && $poster['type'] == 2 && $goods_id){//是商品海报，判断是否设置了独立海报
                //获取每个类型的默认数据
                $is_goods_poster = $goodsSer->getGoodsDetailById($goods_id, 'goods_id,is_goods_poster_open,px_type,poster_data');
                ...
                switch ($poster['type']) {
                    ...
                    case 1:
                        ...
                        if(config('project') == 'shopdds'){
                            ...
                            if ($miniSer->getNewCode()) {
                                $poster['mp_page'] = 'pages/mall/index';
                            }else{
                            $poster['mp_page'] = 'pages/index/index';
                            }
                        }
                        $qr_url = $domain_name . '/wap/pages/mall/index';
                        if ($user_info['member_info']['isdistributor'] == 2 && !empty($user_info['member_info']['extend_code'])) {
                    ...
                    case 2:
                        ...
                        if(config('project') == 'shopdds'){
                            //判断是否启用新小程序链接
                            if ($miniSer->getNewCode()) {
                                $poster['mp_page'] = 'packages/goods/detail';
                            }else{
                            $poster['mp_page'] = 'pages/goods/detail/index';
                            }
                        }
                        $goods_info = $goodsSer->getGoodsDetailById($goods_id, '*', 1);
                        if (empty($goods_info)) {
//                            return ['code' => -1, 'message' => '空的商品信息'];
                        ...
                        //获取sku最小的价格
                        //获取sku最小的价格 用于计算限时折扣
                        $min_sku_price = $goodsSer->getMinSkuPrice($goods_id);
                        $goods_info['price'] = $goodsSer->getPromotionPrice($goods_info['promotion_type'], $goods_id, $min_sku_price, $goods_info['shop_id'], $goods_info['website_id']) ? : $min_sku_price;
                        $qr_url = $domain_name . '/wap/packages/goods/detail?goods_id=' . $goods_id;
                        if ($user_info['member_info']['isdistributor'] == 2 && !empty($user_info['member_info']['extend_code'])) {
                            $qr_url .= '&extend_code=' . $user_info['member_info']['extend_code'] . '&poster_id=' . $poster_id . '&poster_type=1';
                        }
                        break;
                    case 3:
                        ...
                        break;
                    case 4:
                        $qr_url = $domain_name .'/wap/packages/microshop/home?shopkeeper_id='.$user_info['uid'];
                        if(config('project') == 'shopdds'){
                            if ($miniSer->getNewCode()) {
                                $poster['mp_page'] = 'packages/microshop/home';
                            }else{
                            $poster['mp_page'] = 'package/pages/microshop/preview/index/index';
                            }
                        }
                        $image_path = $poster_path . $poster_id . '.' . $poster_config['default_ext'];
...
//                        return ['code' => -1, 'message' => '无效的海报类型'];
            ...
            if (is_file($image_path)) {
                return ['code' => 1, 'poster' => $image_path.'?'.time(), 'width' => $poster_config['default_width'], 'height' => $poster_config['default_height']];
            }
                        ...
                        case 'qr':
                            ...
                            if (isset($qr_url)){
                                ...
                                if ($is_mp) {
                                    ...
                                    $code               = $user_info['member_info']['extend_code'] ?:'';
                                    $goods_id           = $goods_id ?: '';
                                    $poster_id          = $poster_id ?: '';
                                    $poster_type        = $poster['poster_type'] ? : '';
                                    $old_code           = !empty($user_info['member_info']['extend_code']) ? $user_info['member_info']['extend_code'] : -1 ;
                                    $old_goods_id       = $goods_id ? '_' . $goods_id : '';
                                    $old_poster_id      = $poster_id ? '_' . $poster_id : '';
                                    $old_poster_type    = $poster['poster_type'] ? '_' . $poster['poster_type'] : '';
                                    if($poster['type'] == 4){
                                        $poster_uid     = $user_info['uid'] ?: '';
                                        $old_poster_uid = $user_info['uid'] ? '_' . $user_info['uid'] : '';
                                    }
                                    ...
                                    if ($miniSer->getNewCode()) {
                                        ...
                                        if ($poster['type'] == 4 && $poster_uid) {
                                            $scene['shopkeeper_id'] = $poster_uid;
                                        }
                                          $scene = json_encode($scene);
                                    } else {
                                        $scene = $old_code .$old_goods_id. $old_poster_uid .$old_poster_id .$old_poster_type;
                                    }
                                    $params = [
                                        'scene' => $scene,
...
//                                $scene_array['poster_id'] = $poster_id;
     ...
     * 获取二维码 小程序码的链接信息
     */
    public function getQrLinkInfo()
    {
        if(getAddons('miniprogram', $this->website_id)){
            $miniprogramSer =  new MiniProgramSer();
            //获取二维码、太阳码的商品详情路径
            if ($miniprogramSer->getNewCode()) {
                $path_prex = '/';
                $mp_url = config('wap_new_route.mp_url');
            }else{
                $path_prex = '';
                $mp_url = config('wap_old_route.mp_url');
            }
        }
        $wap_url = config('wap_new_route.h5_url');//暂时先用旧的，用新的就切换一下wap_new_route就ok
        return ['mp_url'=>$mp_url, 'wap_url' => $wap_url, 'path_prex' => $path_prex];
    }
    /**
     * 砸金蛋、刮刮乐等等活动的开启与关闭
     * @param $promotion_key
     ...
     * 获取累的保护属性[]
     * @param $obj [对象]
     * @param $attr_name [想要获得的属性名]
     * @return mixed [对应的属性值]
     * @throws \ReflectionException
     */
    public function getObjOfProperty($obj, $attr_name)
    {
        $reflection = new \ReflectionClass($obj);
        $property = $reflection->getProperty($attr_name);
        $property->setAccessible(true);
        $attr_value = $property->getValue($obj);
